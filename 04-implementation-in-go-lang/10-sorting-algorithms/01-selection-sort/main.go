...
// selectionSort function sorts the list of Transformers based on their strength in ascending order
func selectionSort(transformers []Transformer) {
	n := len(transformers)
	...
	for i := 0; i < n-1; i++ {
		...
		for j := i + 1; j < n; j++ {
			if transformers[j].Strength < transformers[minIndex].Strength {
				minIndex = j
			}
		}
		...
		if i != minIndex {
			transformers[i], transformers[minIndex] = transformers[minIndex], transformers[i]
		}
	}
}
...
func runTestSuite() {
	...
	for i, example := range examples {
		...
		selectionSort(example.Transformers)
